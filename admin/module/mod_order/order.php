<?php
session_start();
 if (empty($_SESSION['admin']) AND empty($_SESSION['passwd'])){
  echo "<link href='style.css' rel='stylesheet' type='text/css'>
 <center>Untuk mengakses modul, Anda harus login <br>";
  echo "<a href=../../index.php><b>LOGIN</b></a></center>";
} 
else{
$aksi="module/mod_order/aksi_order.php";
switch($_GET[act]){
  // Tampil Order
  default: 
    echo "<h2>Order</h2>
          <table>
          <tr><th>No Order</th><th>Nama Kustomer</th><th>Tanggal Order</th><th>Jam</th><th>Status</th><th>Aksi</th></tr>";

    $tampil = mysql_query("SELECT * FROM orders,member WHERE orders.id_member=member.id_member ORDER BY id_orders DESC LIMIT 6");
  ...
  case "detailorder":
    $edit = mysql_query("SELECT * FROM orders,member WHERE orders.id_member=member.id_member AND id_orders='$_GET[id]'");
  ...
  while($s=mysql_fetch_array($sql2)){
   ...
   $totalberat  = $totalberat + $subtotalberat; // grand total berat all produk yang dibeli
    ...
    echo "<tr><td>$s[nama]</td><td align=center>$s[berat]</td><td align=center>$s[jumlah]</td>
              <td align=right>$harga</td><td align=right>$subtotal_rp</td></tr>";
  }

  $ongkos=mysql_fetch_array(mysql_query("SELECT * FROM kota,member,orders 
          WHERE member.id_kota=kota.id_kota AND orders.id_member=member.id_member AND id_orders='$_GET[id]'"));
...
echo "<tr><td colspan=4 align=right>Total              Rp. : </td><td align=right><b>$total_rp</b></td></tr>
  ...
  echo "<table border=0 width=500>
        ...
        <tr><td>Nomor Rekening</td><td> : $r[telp]</td></tr>
